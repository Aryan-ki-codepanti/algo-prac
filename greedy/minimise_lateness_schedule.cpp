...
void schedule_minimise_max_lateness(vector<Interval> intervals, int start)
{
	...
	int f = start;
	cout << "\nInterval ID\tStart\tFinish\tLateness" << endl;
...
{
	// s is staring time
	cout << "Enter number of intervals and starting time" << endl;

	int n, s = 0;
	cin >> n >> s;
	vector<Interval> schedules(n);

	cout << "Enter their ids, time lengths and deadlines" << endl;
	int x, y, z;
